...
const DEFAULTS = {
  ...
  touchIcon: path.join(HERE, 'touch-icon.png'),
};
...
function assemble(args) {
  ...
  html = html.replaceAll('__THEME_LIGHT__', escAttr(pal.light.bg)).replaceAll('__THEME_DARK__', escAttr(pal.dark.bg));
  const touchIconB64 = fs.readFileSync(DEFAULTS.touchIcon).toString('base64');
  html = html.replace('__APPLE_TOUCH_ICON__', `data:image/png;base64,${touchIconB64}`);
