...
Known deviations from the spec, all recorded in Global Constraints: recipient validation lives in the runner; the messaging paragraph is composed per turn by `composePrompt`; the hop cap reads parents from host memory and treats an unknown parent as a fresh thread.
